// Code by Matheus Lin
// Spider webs using canvas arcTo function
...
function Controller() {
  var self = this;
  self.canvas = document.getElementById("spiderweb");
  ...
  self.drawWebByParts = function () {
    ...
    var outerRadius =
      Math.min(self.screenWidth / 2, self.screenHeight / 2) + 300;
    ...
    // self.shuffleArray(linesToDraw); // shuffle lines drawn
    ...
    // self.shuffleArray(arcsToDraw); // shuffle arcs order
  ...
  self.init = function () {
    self.color = new Color(64); // Change this value to see a colorful Web
    self.resize();
  };
}
